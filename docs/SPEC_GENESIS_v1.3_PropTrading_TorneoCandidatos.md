# Spec Génesis v1.3 — Sistema de Trading Sistemático para Prop Firms (Torneo de Candidatos)

**Fecha**: 2026-07-11
**Estado**: definitivo (SSoT vigente). Reemplaza íntegramente al v1.2.
...
| `daily_loss_limit` | **5%** — base análoga a §1.3.1 (el mayor de la pérdida contra equity flotante intradía y contra el balance/equity de referencia del corte diario); P3 evalúa sobre equity flotante. *Default conservador — a confirmar contra términos vigentes de FTMO / la corrida operativa.* |
| `daily_reset_time` | **`00:00`** en el `server_tz` de FTMO (típicamente Europe/Prague, CET/CEST). Equivalencia UTC: 23:00 UTC (CET, invierno) / 22:00 UTC (CEST, verano). *Default conservador — `server_tz` y hora exacta a confirmar contra el terminal FTMO / la corrida operativa.* |
| `server_tz` | Europe/Prague (CET/CEST). *Default conservador — a confirmar contra `account_info`/`symbol_info` del terminal FTMO en la corrida operativa (patrón PA-1).* |
| `equity_basis` | `equity` (equity flotante intradía, con cota adicional del balance de referencia del corte — ver §1.1). *A confirmar.* |
| `max_loss_limit` | **10%**, tipo **estático** (ancla: balance inicial). *Default conservador — tipo estático/trailing y ancla a confirmar contra términos vigentes de FTMO.* |
...
lista del terminal FTMO. Los nombres de la columna *Símbolo MT5 esperado en FTMO* son **esperados, a
confirmar por la corrida operativa** (patrón PA-1), nunca definitivos.
...
Nota: los nombres esperados y sus alias derivan del sondeo `out/run_d/probe_mt5.py` (candidatos por
símbolo convencional); FTMO suele sufijar los índices con `.cash`. Estos nombres se confirman contra el
terminal FTMO en la corrida operativa y se corrigen aquí en un backfill posterior si difieren (mismo
mecanismo que The5ers/Issue B). La confirmación del `SymbolFigure` real de oro/majors queda **fuera de
alcance** — ver §2.x.
