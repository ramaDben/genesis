...
def test_monte_carlo_symbol_produce_ambos_metodos_con_metricas_finitas(
    ...
    result = monte_carlo_symbol(ledger, house_rule_fixture, n_paths=1000, seed=42)
...
def test_monte_carlo_symbol_default_block_size_formula(
    ...
    result = monte_carlo_symbol(ledger, house_rule_fixture, n_paths=10, seed=1)
...
def test_monte_carlo_symbol_block_size_explicito_se_respeta(
    ...
    result = monte_carlo_symbol(ledger, house_rule_fixture, n_paths=10, seed=1, block_size=3)
...
def test_monte_carlo_symbol_determinismo(
    ...
    ledger = build_ledger([10.0, -5.0, 8.0, -3.0, 12.0, -6.0, 7.0, -2.0, 9.0, -4.0])
    result1 = monte_carlo_symbol(ledger, house_rule_fixture, n_paths=200, seed=7)
    result2 = monte_carlo_symbol(ledger, house_rule_fixture, n_paths=200, seed=7)
...
def test_monte_carlo_symbol_semillas_distintas_producen_resultados_distintos(
    ...
    ledger = build_ledger([10.0, -5.0, 8.0, -3.0, 12.0, -6.0, 7.0, -2.0, 9.0, -4.0])
    result1 = monte_carlo_symbol(ledger, house_rule_fixture, n_paths=200, seed=1)
    result2 = monte_carlo_symbol(ledger, house_rule_fixture, n_paths=200, seed=2)
    assert not np.array_equal(
        result1.reshuffle.max_drawdown_per_path, result2.reshuffle.max_drawdown_per_path
    )


def test_monte_carlo_symbol_n_paths_no_positivo_lanza(house_rule_fixture: HouseRule) -> None:
    ledger = build_ledger([1.0, -1.0, 2.0])
    with pytest.raises(MonteCarloConfigError):
        monte_carlo_symbol(ledger, house_rule_fixture, n_paths=0, seed=1)


def test_monte_carlo_symbol_block_size_no_positivo_lanza(house_rule_fixture: HouseRule) -> None:
    ledger = build_ledger([1.0, -1.0, 2.0])
    with pytest.raises(MonteCarloConfigError):
        monte_carlo_symbol(ledger, house_rule_fixture, n_paths=10, seed=1, block_size=0)


def test_monte_carlo_symbol_ledger_sin_trades_lanza(house_rule_fixture: HouseRule) -> None:
    ledger = build_empty_ledger()
    with pytest.raises(MonteCarloConfigError):
        monte_carlo_symbol(ledger, house_rule_fixture, n_paths=10, seed=1)
...
def test_monte_carlo_symbol_reshuffle_es_permutacion_sin_reemplazo(
    ...
    ledger = build_ledger(deltas)
    result = monte_carlo_symbol(ledger, house_rule_fixture, n_paths=5, seed=3)
